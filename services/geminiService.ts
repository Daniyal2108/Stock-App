...
const generateFallbackAdvisory = (marketData: MarketData, user: UserProfile | null): string => {
    ...
    const action = marketData.changePercent < -2 ? "WATCH FOR OPPORTUNITY" : (marketData.changePercent > 2 ? "MONITOR CLOSELY" : "HOLD");
    ...
    **Strategy:**
    The asset is currently showing ${trend.toLowerCase()} momentum. Given your '${risk}' risk profile, ${action === 'WATCH FOR OPPORTUNITY' ? 'this volatility may present opportunities to monitor.' : 'maintain current positions and monitor volume.'}
...
export const getMarketAnalysis = async (
  ...
  try {
    const userContext = user 
      ? `User Profile: ${user.name}, Risk: ${user.riskTolerance}, Goal: ${user.goal}.` 
...
export const chatWithAnalyst = async (
  ...
  user: UserProfile | null
    ...
    try {
        const userContext = user 
            ? `USER CONTEXT: Name: ${user.name}. Risk Tolerance: ${user.riskTolerance}. Investment Goal: ${user.goal}. Cash Balance: $${user.balance}.` 
        ...
        const systemInstruction = `
            ...
            GUIDELINES:
            1. Provide market analysis and insights without executing trades.
            ...
            5. Disclaimer: Always end with a brief "Not financial advice" note. This is for informational purposes only.
        `;
